...
int main(int argc, char *argv[]) {
	int cantidad_puntos;
	char a_mano = 'x';
	double a= 3.14159265359/2 ,b=3.14159265359;  // NOS SIRVE PARA INGRESAR VALORES DE PUNTOS MAS COMPLEJOS (pi, e, etc)
	double integral;
	double c0,c1,c2,c3,c4,c5,x0,x1,x2,x3,x4,x5;
	printf("\nDesea cargar los puntos a mano? (s / n)\n");
	scanf("%c",&a_mano);
	if (a_mano == 's'){
		printf("\nIngresar a\n");
		scanf("%lf",&a);
	...
	case 2:
		c0 = 1;  	// Factores de ponderacion
		c1 = 1; 	// Factores de ponderacion	
		x0 = - 0.577350269;		// Argumentos de la funcion
		x1 = 0.577350269;		// Argumentos de la funcion
