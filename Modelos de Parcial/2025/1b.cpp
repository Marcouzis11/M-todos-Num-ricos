#include <stdio.h>
#include <math.h>

#define MAX_ITER 10000
#define XEXACTO 2

double funcion(double x);
void biseccion();

int main() {
    biseccion();
    return 0;
}

double funcion(double x) {
    return ((x+1)/(x+4))-0.25*x;
}

void biseccion() {
    double a, b, tolerancia;
    double c, cViejo, error, errorPorcExacto;
    double fa, fb, fc;
    int iterator = 0;

    printf("Ingrese el límite inferior del intervalo: ");
    scanf("%lf", &a);
    printf("Ingrese el límite superior del intervalo: ");
    scanf("%lf", &b);
    printf("Ingrese la tolerancia: ");
    scanf("%lf", &tolerancia);

    /* Si el intervalo viene invertido lo acomoda, si no (b-a) queda negativo */
    if (a > b) {
        double aux = a;
        a = b;
        b = aux;
        printf("\nAviso: se intercambiaron los extremos del intervalo\n");
    }

    fa = funcion(a);
    fb = funcion(b);

    /* La función puede no estar definida en los extremos (log de x <= 0) */
    if (!isfinite(fa) || !isfinite(fb)) {
        printf("\nLa función no está definida en algún extremo del intervalo\n");
        return;
    }

    /* Raíz exacta en un extremo: el producto daría 0 y no entraría al bucle */
    if (fa == 0.0) {
        printf("\nLa raíz exacta es el extremo inferior: %.10lf\n", a);
        return;
    }
    if (fb == 0.0) {
        printf("\nLa raíz exacta es el extremo superior: %.10lf\n", b);
        return;
    }

    if (fa * fb < 0.0) {

        c = a;   /* semilla, para que la primera resta tenga sentido */

        do {
            cViejo = c;
            c = (a + b) / 2;
            fc = funcion(c);

            //error = fabs(c - cViejo);
            error = (fabs(c - cViejo)/(0.5*fabs(c + cViejo)))*100;
            iterator++;

            if (fa * fc > 0) {
                a = c;
                fa = fc;
            } else if (fa * fc < 0) {
                b = c;
            } else {
                error = 0.0;   /* f(c) == 0, la raíz es exacta */
                break;
            }

        } while (error > tolerancia && iterator < MAX_ITER);

        if (iterator >= MAX_ITER) {
            printf("\nNo se alcanzó la tolerancia en %d iteraciones\n", MAX_ITER);
        }

        errorPorcExacto = (fabs(XEXACTO - c)/fabs(XEXACTO))*100;
        printf("\nResultado de la raíz: %.10lf", c);
        printf("\nError estimado: %.10lf", error);
        printf("\nError porcentual exacto: %.10lf", errorPorcExacto);
        printf("\nCantidad de iteraciones: %d\n", iterator);

    } else {
        printf("\nNo hay raíz o hay un número par de ellas\n");
    }
}
